...
import { getStartedCars } from '../../store/selectors';
...
const GarageItem = ({ className, name, color, id }: GarageItemTypes): JSX.Element => {
  const dispatch = useDispatch();

  const startedCars = useSelector(getStartedCars);
  const isStarted = startedCars.find(startedCar => startedCar.id === id)?.isStarted;
  ...
  return (
    <div className={`garage-item ${className}`}>
      ...
      <div className="garage-item__bottom">
        ...
        <svg className="garage-item__flag" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
